...
class ControladorUsuarios
{
        ...
        if (isset($_POST['registroNombre'])) {
            if (
                ...
                if ($respuesta == 'ok') {
                    ...
                    $mail->setFrom(
                        'cursos@tutorialesatualcance.com',
    ...
    public function ctrIngresoUsuario()
    {
        if (isset($_POST['ingresoEmail'])) {
            if (
                ...
                $respuesta = ModeloUsuarios::mdlMostrarUsuario(
                    $tabla,
                    $item,
                    $valor
                );
    ...
    public static function ctrRegistroRedesSociales($datos)
    {
        ...
        $verificarExistenciaUsuario = ModeloUsuarios::mdlMostrarUsuario(
            $tabla,
            $item,
            $valor
        );
        ...
        if ($verificarExistenciaUsuario) {
            ...
            $registrarUsuario = ModeloUsuarios::mdlRegistroUsuario(
                $tabla,
                $datos
            );
        }
        ...
        if ($emailRepetido || $registrarUsuario == 'ok') {
            $traerUsuario = ModeloUsuarios::mdlMostrarUsuario(
                $tabla,
                $item,
                $valor
            );
            ...
            if ($traerUsuario['modo'] == 'facebook') {
                ...
                echo 'ok';
            } elseif ($traerUsuario['modo'] == 'google') {
                $_SESSION['validarSesion'] = 'ok';
                $_SESSION['id'] = $traerUsuario['id_u'];
                $_SESSION['nombre'] = $traerUsuario['nombre'];
                $_SESSION['foto'] = $traerUsuario['foto'];
    ...
    public function ctrCambiarFotoPerfil()
    {
        if (isset($_POST['idUsuarioFoto'])) {
            ...
            if (
                isset($_FILES['cambiarImagen']['tmp_name']) &&
                !empty($_FILES['cambiarImagen']['tmp_name'])
            ) {
                list($ancho, $alto) = getimagesize(
                    $_FILES['cambiarImagen']['tmp_name']
                );
                ...
                $directorio =
                    'backend/vistas/img/usuarios/' . $_POST['idUsuarioFoto'];
            ...
            $actualizarFotoPerfil = ModeloUsuarios::mdlActualizarUsuario(
                $tabla,
                $id,
                $item,
                $valor
    ...
    public function ctrFormularioContactenos()
    {
        if (isset($_POST['mensajeContactenos'])) {
            if (
                preg_match(
                    '/^[a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/',
                    $_POST['nombreContactenos']
                ) &&
                preg_match(
                    '/^[a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/',
                    $_POST['apellidoContactenos']
                ) &&
                preg_match('/^[0-9- ]+$/', $_POST['movilContactenos']) &&
                preg_match(
                    '/^[^0-9][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[@][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{2,4}$/',
                    $_POST['emailContactenos']
                ) &&
                preg_match(
                    '/^[?\\¿\\!\\¡\\:\\,\\.\\a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/',
                    $_POST['mensajeContactenos']
                )
            ) {
                /*=============================================
				VERIFICACIÓN CORREO ELECTRÓNICO
                ...
                $mail->msgHTML(
                    '<div style="width:100%; background:#eee; position:relative; font-family:sans-serif; padding-bottom:40px">
                        ...
                        $_POST['apellidoContactenos'] .
                        '</h4>
								<h4 style="font-weight:100; color:#999; padding:0px 20px;">Móvil: ' .
                        $_POST['movilContactenos'] .
                        '</h4>
								<h4 style="font-weight:100; color:#999; padding:0px 20px;">Email: ' .
                        $_POST['emailContactenos'] .
                        '</h4>
